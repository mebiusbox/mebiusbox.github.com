...
void getDirectionalDirectLightIrradiance(const in DirectionalLight directionalLight, const in GeometricContext geometry, out IncidentLight directLight) {
  ...
  directLight.distance = 0.0;
  directLight.attenuation = 0.0;
...
void getPointDirectLightIrradiance(const in PointLight pointLight, const in GeometricContext geometry, inout IncidentLight directLight) {
  vec3 L = pointLight.position - geometry.position;
  directLight.direction = normalize(L);
  
  directLight.distance = length(L);
  directLight.attenuation = punctualLightIntensityToIrradianceFactor(directLight.distance, pointLight.distance, pointLight.decay);
  if (testLightInRange(directLight.distance, pointLight.distance)) {
    directLight.color = pointLight.color;
    directLight.color *= directLight.attenuation;
    directLight.visible = true;
  } else {
...
void getSpotDirectLightIrradiance(const in SpotLight spotLight, const in GeometricContext geometry, inout IncidentLight directLight) {
  vec3 L = spotLight.position - geometry.position;
  directLight.direction = normalize(L);
  directLight.distance = length(L);
  directLight.attenuation = punctualLightIntensityToIrradianceFactor(directLight.distance, spotLight.distance, spotLight.decay);
  
  float angleCos = dot(directLight.direction, spotLight.direction);
  
  if (all(bvec2(angleCos > spotLight.coneCos, testLightInRange(directLight.distance, spotLight.distance)))) {
    float spotEffect = smoothstep(spotLight.coneCos, spotLight.penumbraCos, angleCos);
    directLight.color = spotLight.color;
    directLight.color *= spotEffect * directLight.attenuation;
    directLight.visible = true;
  } else {
    directLight.color = vec3(0.0);
    directLight.visible = false;
...
void RE_Direct(const in IncidentLight directLight, const in GeometricContext geometry, const in Material material, inout ReflectedLight reflectedLight) {
  ...
  float attenuation = directLight.attenuation;
  if (directLight.distance <= 0.0) {
    ...
    attenuation = punctualLightIntensityToIrradianceFactor(distance, translucentCutoff, 1.0);
  }

  vec3 strength = translucentColor * texture2D(translucentMap, vUv).r * translucentStrength;
  vec3 light = normalize(directLight.direction + (geometry.normal * translucentDistortion));
  float dotLT = pow(saturate(dot(geometry.viewDir, -light)), translucentPower) * translucentScale;
  vec3 LT = attenuation * (dotLT + translucentAmbient) * strength;
  vec3 subsurfaceColor = material.diffuseColor * directLight.color * LT;
  reflectedLight.directDiffuse += mix(DiffuseBRDF(material.diffuseColor) * irradiance, subsurfaceColor, translucency);
  // reflectedLight.directDiffuse += material.diffuseColor * directLight.color * LT;
}
